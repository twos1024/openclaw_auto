...
function buildConfigRuntimeIssue(runtime: RuntimeDiagnostics): BackendError {
  if (runtime.mode === "browser-preview") {
    return {
      ...
      suggestion: "请使用 ClawDesk 桌面应用或 `npm run electron:dev` 后再读取和保存真实配置。",
      details: {
  ...
  return {
    ...
    message: "当前已进入桌面窗口，但 Electron IPC 通道不可用，无法读取本地 OpenClaw 配置。",
    suggestion: "请重启或重新安装 ClawDesk；若问题持续，请检查 preload 脚本是否已正确加载。",
    details: {
      runtimeMode: runtime.mode,
      bridgeSource: runtime.bridgeSource,
    },
...
export const configService = {
  async readConfig(): Promise<ConfigLoadResult> {
    ...
    if (result.error?.code === "E_PREVIEW_MODE" || result.error?.code === "E_TAURI_UNAVAILABLE" || result.error?.code === "E_IPC_UNAVAILABLE") {
