...
function classifyErrorStage(error?: BackendError): InstallPhaseId {
  ...
  if (code === "E_PREVIEW_MODE" || code === "E_TAURI_UNAVAILABLE" || code === "E_IPC_UNAVAILABLE") {
    return "prerequisite";
  }
...
function buildIssueFromError(error?: BackendError): InstallIssue | null {
  ...
  if (error.code === "E_TAURI_UNAVAILABLE" || error.code === "E_IPC_UNAVAILABLE") {
    return {
      stage: "prerequisite",
      ...
      message: error.message,
      suggestion: error.suggestion,
      step: "initialize the IPC command bridge",
      sample: null,
    };
  }
