...
async function invokeAction(command: string): Promise<ServiceActionResult> {
  ...
  if (runtime.mode !== "electron-runtime-available") {
    return {
      status: "error",
      detail:
        runtime.mode === "browser-preview"
          ? "Gateway controls are unavailable in browser preview mode."
          : "Gateway controls are unavailable because the Electron IPC bridge is not ready in this desktop runtime.",
      suggestion:
        runtime.mode === "browser-preview"
          ? "Start ClawDesk in the Electron desktop shell to run local process commands."
          : "Relaunch or reinstall ClawDesk and verify the preload script is loaded correctly.",
      code: runtime.mode === "browser-preview" ? "E_PREVIEW_MODE" : "E_IPC_UNAVAILABLE",
    };
  }
...
export const serviceService = {
  async getGatewayStatus(): Promise<GatewayStatus> {
    ...
    if (runtime.mode === "electron-runtime-unavailable") {
  ...
  async probeDashboardEndpoint(address: string): Promise<DashboardProbeResult> {
    ...
    if (runtime.mode !== "electron-runtime-available") {
      return {
        ...
        detail:
          runtime.mode === "browser-preview"
            ? "Endpoint probe is only available in the Electron runtime."
            : "Endpoint probe is unavailable because the Electron IPC bridge is not ready.",
      };
    }
