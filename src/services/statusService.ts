...
function buildPreviewOverview(updatedAt: string): OverviewStatus {
...
function buildRuntimeSection(envResult: CommandResult<DetectEnvData>, updatedAt: string): OverviewSection {
...
function buildInstallSection(envResult: CommandResult<DetectEnvData>, updatedAt: string): OverviewSection {
  if (!envResult.success || !envResult.data) {
    return buildSection(
      "openclaw-install",
      "OpenClaw 安装",
...
function buildNextActions(sections: Pick<OverviewStatus, "install" | "config" | "service" | "settings">): OverviewAction[] {
